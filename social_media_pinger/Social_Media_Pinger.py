...
from config.Default_Ordered import DefaultOrderedDict
...
class Social_Media_Tracker(object):
    ...
    def ping_twitter(self, *args, **kwargs):
        ...
        #Look up twitter id in csv and write new follower_count
            for i,handle in enumerate(self.data_dict['twitter_handle']):
                if handle:
                    user = api.get_user(auth, screen_name=handle)
                    ...
                    #print 'HANDLE:',handle
                    self.data_dict['followers '+self.stamp].append('-')
        except Exception, error:
